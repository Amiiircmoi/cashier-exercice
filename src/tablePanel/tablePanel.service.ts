...
export class TablePanelService {
  ...
  async getTable(id: number): Promise<Table> {
    ...
    // load order
    if (this._table && this._table.invoice && this._table.invoice.id) {
      const order = await this.service.findOneInvoice(this._table.invoice.id);
      this._table.invoice = order;
    }
    return this.table;
  }
  ...
  async addProductInvoice(id_product: number, quantity: number) {
    const product = await this.service.findOneProduct(id_product);
    this.table.addProductInvoice(product, quantity);
    await this.service.saveInvoice(this.table.invoice);
  }
